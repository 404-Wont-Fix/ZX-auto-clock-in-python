...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            errorIfEnabled('Worker 顶级错误捕获:', error);
            return jsonResponse({
                success: false,
...
async function handleClockin(request, env) {
    try {
        ...
        logIfEnabled(`开始处理用户 ${username} 的 ${clockin_type} 打卡请求`);
    ...
    try {
        ...
        if (clockinType === 'all' || clockinType === 'home') {
            ...
            try {
                ...
                errorIfEnabled('首页签到失败:', e);
            }
            if (clockinType === 'all') {
                logIfEnabled('首页签到完成，等待2秒后执行运动打卡...');
                await delay(2000);
            }
        }
        ...
        if (clockinType === 'all' || clockinType === 'sports') {
            ...
            while (retryCount <= maxRetries) {
                try {
                    ...
                    if (retryCount < maxRetries) {
                        logIfEnabled(`运动打卡失败，${1500}ms 后重试 (${retryCount + 1}/${maxRetries})...`);
                        await delay(1500);
                        retryCount++;
                    } else {
                        sportsResult = { success: false, message: `运动打卡异常: ${e.message}`, data: null };
                        errorIfEnabled('运动打卡失败:', e);
                    }
                }
            }
            ...
            if (clockinType === 'all') {
                logIfEnabled('运动打卡完成，等待2秒后执行日精进打卡...');
                await delay(2000);
            }
        }
